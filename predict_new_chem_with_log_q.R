...
for(i in 1:4){
  ...
  temp$Morphology=factor(temp$Morphology,exclude = c("---",""),
                         levels = c("A15","BCC","GYR","HEX","HEX/LAM","LAM","Sigma","DIS"))
...
est_method="all_samples" #c("one_sample","all_samples","DIS_samples","manually_set")
if(est_method=="one_sample"){
  ##use one sample estimate parameters
  ind_optim=143#36
  param=get_param(input=log10(q_all[truncate_ind]),output=log10(I_q[ind_optim,truncate_ind]),kernel_type=kernel_type,homo=homo,error_var=log10_var[ind_optim,truncate_ind])
}else if(est_method=="all_samples"){
  ##use all training samples to estimate parameters
  outputs_train=log10(I_q[-new_chem_ind,])
  m_all=optim(c(log(1),log(.1)),get_log_lik_sum,input=log10(q_all[truncate_ind]),outputs=outputs_train[,truncate_ind],kernel_type=kernel_type,
  ...
  m_DIS=optim(c(log(1),log(.1)),get_log_lik_sum,input=log10(q_all[truncate_ind]),outputs=outputs_train[DIS_train_ind,truncate_ind],kernel_type=kernel_type,
...
param
#smooothing
smoothed_log_hetero=smooth_intensity_fix_param(q_all=log10(q_all),log_I=log10(I_q),param=param,kernel_type=kernel_type,homo=homo,log_var=log10_var)


##2. peak detect
peak_height_cutoff=.05
peak12_not_close=T
...
ggplot(max_pred_prob_dat, aes(Type, max_prob))+
  ...
  geom_dotplot(aes(fill=Type),stroke=.3,binaxis = "y", stackdir ="center",binwidth=.008,dotsize=2.0,stackratio=.7)+
...
for(i in 1:length(plot_ind)){
  plot_i=plot_ind[i]
  plot(q_all,log10(I_q[test_ind,][plot_i,]),pch=20,cex=.5,xlab='q',ylab=expression(paste(log[10]," [I(q)] (a.u.)")),cex.main=1,
       main=paste("True:",test_label[plot_i],", Pred:", pred_label[plot_i]))
...
pdf(paste0("all ",class_select," samples_with_log10_q.pdf"),height=10,width=12)
...
# for(i in 1:length(phase_ind)){
#   plot_i=phase_ind[i]
#   plot(log10(q_all),log10(I_q[plot_i,]),pch=20,cex=.5,xlab='q',ylab="I(q)",cex.main=1,
#        main=paste0(plot_i,"-True: ",label[plot_i]),type='l')
#   lines(log10(q_all),smoothed_log_hetero[plot_i,],col=4)
#   abline(v=log10(q_all[peak_res_hetero$peak[plot_i,]]),col=2,lty=4)
# }
dev.off()
