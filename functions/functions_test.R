...
compute_height=function(height_increase,height_decrease,increase_width,decrease_width){
  height_here=(height_increase/increase_width*(increase_width+decrease_width)+height_decrease/decrease_width*(increase_width+decrease_width))/4
}
...
peak_detect=function(q_all,smoothed_log,peak_height_cutoff,peak12_not_close=F,truncate_range){#,peak_slope_cutoff=0){#grad2_cutoff
  ...
  for(i in 1:dim(smoothed_log)[1]){
    ...
    if(length(potential_peak)>0){
      ...
      increase_width=potential_peak-start_of_increase
      ...
      #remove the potential peaks with width < width_control
      width_control=3
      smooth_out_int=which(decrease_width<width_control |increase_width<width_control)
      if(length(smooth_out_int)>0){
        for(i_smooth in rev(smooth_out_int)){
          if(i_smooth<length(potential_peak)){
            if(decrease_width[i_smooth]<width_control){
            ...
            }else if(increase_width[i_smooth]<width_control){
              start_of_increase=start_of_increase[-i_smooth]
              potential_peak=potential_peak[-i_smooth]
              end_of_decrease=end_of_decrease[-(i_smooth-1)]
            }
      ...
      height_here=compute_height(height_increase,height_decrease,increase_width,decrease_width)#pmax(height_increase,height_decrease)
      index_peak=which(height_here>peak_height_cutoff)#  & peak_slope>peak_slope_cutoff)# & grad2<grad2_cutoff) 
      ...
      if(length(index_peak)>0){
        ...
        height_decrease=height_decrease[index_peak]
        height_here=height_here[index_peak]
        ...
        if(peak12_not_close & length(potential_peak)>1 & (potential_peak[2]-potential_peak[1])<10){
          ...
          if(smoothed_log[i,end_of_decrease[1]]<smoothed_log[i,end_of_decrease[2]]){
            end_of_decrease=end_of_decrease[-2]
          }else{
            end_of_decrease=end_of_decrease[-1]
          }
          height_increase=height_increase[-1]
          height_increase[1]=smoothed_log[i,potential_peak[1]]-smoothed_log[i,start_of_increase[1]]
          height_decrease=height_decrease[-1]
          height_decrease[1]=smoothed_log[i,potential_peak[1]]-smoothed_log[i,end_of_decrease[1]]
          height_here=height_here[-1]
          height_here[1]=max(height_increase[1],height_decrease[1])
        }
        ...
        peak_height[i,1:length(potential_peak)]=height_here
...
get_data=function(model_peak,model_label,n_peak,
                  ...
                  add_frac=F,add_weight=F,add_weight_D=F,add_weight_F=F,
                  peak1_grad2_value=NA,peak1_grad2_width_to_zero=NA,#peak1_up_down_width=NA,
                  frac=NA,weight=NA,weight_D=NA,weight_F=NA){
...
classification_train_test=function(train_input,train_output,test_input,test_output,model_used=c('bagging','rforest','xgboost')){
  ...
  return(list(class_pred_label=class_pred_label,pred_prob=m_pred))
}
