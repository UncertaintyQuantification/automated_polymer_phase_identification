...
////Get_Q_K  
// [[Rcpp::export]] 
List Get_Q_K_hetero(const List GG,const List  W,const Eigen::MatrixXd C0,const double VV, const Eigen::VectorXd error_var){ 
...
////Get_log_det_S2
// [[Rcpp::export]] 
List Get_log_det_S2_hetero(const Eigen::VectorXd param,const bool have_noise,const Eigen::VectorXd delta_x,const Eigen::VectorXd output,
                           const  String kernel_type, const Eigen::VectorXd error_var){
  ...
  Q_K=Get_Q_K_hetero(GG,W,W0,VV,error_var);
...
// [[Rcpp::export]] 
List Get_C_R_K_Q_hetero(const VectorXi index, const List GG,const List  W,const Eigen::MatrixXd C0, double VV, const Eigen::VectorXd error_var){ 
...
// [[Rcpp::export]] 
List Kalman_smoother_hetero(const VectorXd param,const bool have_noise,const VectorXi index_obs, 
  ...
  List C_R_K_Q=Get_C_R_K_Q_hetero(index_obs, GG,W, W0, VV,error_var);
  ...
  Var=((Var.array()+VV*error_var.array())*sigma_2_hat).matrix();
...
// [[Rcpp::export]] 
VectorXd R_times_z_pre_constructed(const Eigen::VectorXd z, const Eigen::VectorXd sqrt_Q, const Eigen::MatrixXd K, const List GG){
  ...
  //Sep 2022
  // 1. L^t z
  Eigen::VectorXd tilde_z=Eigen::VectorXd::Zero(n);
  
  tilde_z(n-1)=sqrt_Q(n-1)*z(n-1);
  ...
  if(n>=2){
    ...
    tilde_z(n-2)=sqrt_Q(n-2)*(u(0)*z(n-1)+z(n-2)); // this is for Matern 2.5
    if(n>=3){ 
      //Nov 2022
      ...
      g=GG_matrix.row(0)*z(n-1);
      for(int t=n-3;t>=0;t--){
        GG_matrix_plus_1=GG[t+1];
        GG_K=GG_matrix_plus_1*K.row(t).transpose();
        g_GG_K=g*GG_K;
        tilde_z(t)=sqrt_Q(t)*(g_GG_K(0)+GG_K(0)*z(t+1)+z(t));
        g=g*GG_matrix_plus_1+GG_matrix_plus_1.row(0)*z(t+1);
      }
    }
    ...
    /*
     GG_matrix=GG[n-1];
     Eigen::VectorXd u=GG_matrix*K.row(n-2).transpose();
     tilde_z(n-2)=sqrt_Q(n-2)*(u(0)*z(n-1)+z(n-2));
     if(n>=3){ 
     Eigen::MatrixXd tilde_G=GG_matrix*z(n-1);
     Eigen::MatrixXd GG_matrix_plus_1;
     Eigen::VectorXd tilde_u;
     for(int t=n-3;t>=0;t--){
     GG_matrix=GG[t];
     GG_matrix_plus_1=GG[t+1];
     u=GG_matrix_plus_1*K.row(t).transpose();
     tilde_u=GG_matrix_plus_1*tilde_G*K.row(t).transpose();
     tilde_z(t)=sqrt_Q(t)*(tilde_u(0)+u(0)*z(t+1)+z(t) );
     tilde_G= GG_matrix_plus_1*tilde_G+GG_matrix_plus_1*z(t+1);
     }
     }
     */
  }
  
  //y_new[n1]=sqrt_Q[n1]*z[n1]
  
  //2. L tilde_z
  
  //Eigen::MatrixXd m=Eigen::VectorXd::Zero(n);
  Eigen::VectorXd m=Eigen::VectorXd::Zero(GG_matrix.cols());
  ...
  for(int t=0;t<n;t++){
    GG_matrix=GG[t];
    a=GG_matrix*m;
    output[t]=a[0]+tilde_z[t]*sqrt_Q[t];
...
// [[Rcpp::export]] 
VectorXd R_inv_times_z_pre_constructed(const Eigen::VectorXd z, const Eigen::VectorXd sqrt_Q, const Eigen::MatrixXd K, const List GG){
  ...
  for(int t=0;t<n;t++){
    GG_matrix=GG[t];
    a=GG_matrix*m;
    tilde_z[t]=(z[t]-a[0])/sqrt_Q[t];//a[0]+z[t]*sqrt_Q[t];
    
    //Y_minus_a_1[t]=(tilde_z[t]-a[0]);
    
    // Y_minus_a_1_scaled_vec[t]=(tilde_z[t]-a[0])/sqrt_Q[t];
    m=a+K.row(t).transpose()*(tilde_z[t]*sqrt_Q[t]);
  }
  ...
  output(n-1)=tilde_z(n-1)/sqrt_Q(n-1);
  
  if(n>=2){
    GG_matrix=GG[n-1];
    Eigen::VectorXd u=GG_matrix*K.row(n-2).transpose();
    output(n-2)=tilde_z(n-2)/sqrt_Q(n-2)-u(0)*output(n-1);//sqrt_Q(n-2)*(u(0)*tilde_z(n-1)+tilde_z(n-2)); // this is for Matern 2.5
    if(n>=3){ 
      //Aug 2023
      Eigen::MatrixXd GG_matrix_plus_1;
      Eigen::MatrixXd g;
      ...
      for(int t=n-3;t>=0;t--){
        ...
        GG_K=GG_matrix_plus_1*K.row(t).transpose();
        g_GG_K=g*GG_K;
        output(t)=tilde_z(t)/sqrt_Q(t)-g_GG_K(0)-GG_K(0)*output(t+1);//sqrt_Q(t)*(g_GG_K(0)+GG_K(0)*tilde_z(t+1)+tilde_z(t));
