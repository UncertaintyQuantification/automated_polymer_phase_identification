...
set.seed(1)
N=length(label)
folds_all=get_folds(nfold=nfold,size=N)
...
for(i_fold in 1:nfold){
  ...
  # par(mfrow=c(1,1))
  # i=138#211#276
  # plot(log10(I_q[i,]),pch=20,main=label[i])
  # lines(smoothed_log_hetero[i,],col=3)
  # abline(v=peak_res_hetero$peak[i,],col=2)
  # lines(log10(I_q[i,])-10*sqrt(log10_var[i,]),lty=2,col=4)
  # lines(log10(I_q[i,])+10*sqrt(log10_var[i,]),lty=2,col=4)
  ...
  #         main=paste('Variable Importance of random forest model\nfor predicting fold',i_fold),names.arg=var.name[import_sort$ix])
  # #varImpPlot(model_hetero$model)
}

mean(accuracy_fold)
sum(label != pred_label_all)
# #plot the samples with no peak detected
# peak0_detect_ind=which(is.na(pred_prob_all[,1]))
# matplot(t(log(I_q[peak0_detect_ind,])),type='l')
...
for(i in 1:length(wrong_ind)){
  ...
  plot(q_all,log10(I_q[plot_i,]),pch=20,cex=.5,xlab='q',ylab="I(q)",cex.main=1,
       #main=paste("True:",label[plot_i],", Pred:", pred_label_all[plot_i]))
...
# plot mislabeled samples
pdf("mislabeled.pdf", width=3*1.5, height=1.5)
par(mfrow=c(1,3), mgp=c(2,1,0), mar=c(3,3,1.2,.5)+.1)
for(plot_i in c(138,139,165)){
  plot(q_all,log10(I_q[plot_i,]),type='l',lwd=1.5,xlab='q',ylab=expression(paste(log[10]," [I(q)] (a.u.)")),cex.main=.8,
       main=paste("Label:",label[plot_i],", True/Pred:", pred_label_all[plot_i]))
       #main=paste(plot_i,"- True:",label[plot_i],", Pred:", pred_label_all[plot_i]))
  lines(q_all,smoothed_log_with_test[plot_i,],col=5)
  abline(v=q_all[peak_with_test[plot_i,]],col=7,lty=4,lwd=1.5)
}
dev.off()
...
#max_prob_correct[is.na(max_prob_correct)]=1.0 #the prob for DIS is 100%
...
ggplot(max_pred_prob_dat, aes(Type, max_prob))+
  ...
  #coord_flip()
#scale_fill_manual(values = c("#8dd3c7","#1b9e77", "#ffed6f", "#E69F00", "#e41a1c"))+
dev.off()

table(folds_all[wrong_ind])
mean(apply(pred_prob_all,1,max)<.7)

#plot all samples with max prob smaller than some threshold:
small_prob_ind=which(apply(pred_prob_all,1,max)<.8)
length(small_prob_ind)
par(mfrow=c(4,5), mgp=c(2,1,0), mar=c(3,3,2,.5)+.1)
for(i in 1:length(small_prob_ind)){
  plot_i=small_prob_ind[i]
  plot(q_all,log10(I_q[plot_i,]),pch=20,cex=.5,xlab='q',ylab="log I(q)")
  title(paste("True:",label[plot_i],", Pred:", pred_label_all[plot_i]),col.main=ifelse(plot_i %in% wrong_ind,6,1),cex.main=1)
...
correct_pred_prob_dat=data.frame(max_prob=max_prob_correct,phase=label[-wrong_ind])
ggplot(correct_pred_prob_dat, aes(phase, max_prob))+
  #geom_boxplot(alpha=.3)+
  geom_violin(aes(color=phase),scale = "area")+#,alpha=.3
  #scale_color_manual(values = c("#14775a", "#E69F00"))+
  geom_dotplot(aes(fill=phase),stroke=.3,binaxis = "y", stackdir ="center",binwidth=.003,dotsize=2.5,stackratio=.5)+#
  #scale_fill_manual(values = c("#8dd3c7","#f8e66a"))+
  ggtitle('Predicted Prob for Correct Prediction')+
  ylab("Max Prob")+
  theme_linedraw()+
  theme(legend.position = "none",axis.title.x=element_blank(),
        legend.text=element_text(size=10),axis.text=element_text(size=12),axis.title=element_text(size=13))

#prob of incorrectly predicted by morphology
pred_prob_for_true_label = pred_prob_all[cbind(1:N, label)]
pred_prob_for_true_label[which(is.na(pred_prob_for_true_label))]=1
pred_prob_for_pred_label = pred_prob_all[cbind(1:N, pred_label_all)]
pred_prob_for_pred_label[which(is.na(pred_prob_for_pred_label))]=1
result_vector <- rep("correct", N)
result_vector[wrong_ind] <- "incorrect"
data_prob=data.frame(pred_prob_for_true_label=pred_prob_for_true_label,label=label,
                     pred_prob_for_pred_label=pred_prob_for_pred_label,pred_label=pred_label_all,
                     result = result_vector)
colors_plot=c('#ed7118','#13528f','#117833','#9b1129','#a04dba','#969696')

ggplot(data_prob, aes(pred_label, pred_prob_for_pred_label))+
  #geom_boxplot(alpha=.3)+
  geom_violin(aes(color=pred_label),scale = "area")+#,alpha=.3
  scale_color_manual(values = colors_plot)+
  geom_dotplot(aes(fill=result),stroke=.3,binaxis = "y", stackdir ="center",binwidth=.004,dotsize=2.5,stackratio=.5)+#
...
ggplot(data_prob, aes(label, pred_prob_for_true_label))+
  #geom_boxplot(alpha=.3)+
  geom_violin(aes(color=label),scale = "area")+#,alpha=.3
  scale_color_manual(values = colors_plot)+
  geom_dotplot(aes(fill=result),stroke=.3,binaxis = "y", stackdir ="center",binwidth=.004,dotsize=2.5,stackratio=.5)+#
  scale_fill_manual(values = c("#8dd3c7", "#e41a1c"))+
  ylab("Prob")+
...
###plot all
pdf('all_with_mode_Dec_10.pdf')
par(mfrow=c(4,4), mgp=c(2,1,0), mar=c(3,3,2,.5)+.1)
for(i in 1:dim(I_q)[1]){
  plot_i=i
  plot(q_all,log(I_q[plot_i,]),pch=20,cex=.5,xlab='q',ylab="I(q)",cex.main=1,
